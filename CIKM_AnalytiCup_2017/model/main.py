...
def check_code(mode, gru_mode):
    if mode == 'simple':
        train_df = pd.read_csv('/data/yuyang/weather/data/data_shenzhen/data_processed/train_percentile.csv')
        test_df = pd.read_csv('/data/yuyang/weather/data/data_shenzhen/data_processed/testB_percentile.csv')
        train_add = pd.read_csv('/data/yuyang/weather/data/data_shenzhen/data_processed/train_old_wind_4240.csv')
        testA_add = pd.read_csv('/data/yuyang/weather/data/data_shenzhen/data_processed/testB_old_wind_4240.csv')
        train_1ave8extend = pd.read_csv('/data/yuyang/weather/data/data_shenzhen/data_processed/train_new_wind_1ave_8extend.csv')
        test_1ave = pd.read_csv('/data/yuyang/weather/data/data_shenzhen/data_processed/testB_new_wind_1ave_8extend.csv')
    else:
        trainfile = '/data/yuyang/weather/data/data_shenzhen/CIKM2017_train/train'
        testBfile = '/data/yuyang/weather/data/data_shenzhen/CIKM2017_testB/CIKM2017_testB/test'
    ...
    np.savetxt("/data/yuyang/weather/result/shenzhen/" + "xgb_pre-{}.csv".format(daytime), result_xgb)
    ...
    np.savetxt("/data/yuyang/weather/result/shenzhen/" + "rf_pre-{}.csv".format(daytime), result_rf)
    print('#rf model has been done')

    result_bigru = bigru.BiGRU_train(train_df, test_df, valid, gru_mode).reshape(2000)
    np.savetxt("/data/yuyang/weather/result/shenzhen/" + "bi-gru_pre-{}.csv".format(daytime), result_bigru)
    print('#bigru model has been done')

    ensemble = (result_xgb + result_rf + result_bigru) / 3.0

    np.savetxt("/data/yuyang/weather/result/shenzhen/" + "ensemble-{}.csv".format(daytime), ensemble)
